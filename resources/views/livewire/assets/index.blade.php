        ...
        <x-asset-manager-page-actionbar :breadcrumbs="[
            ...
            <x-slot name="actions">
                ...
                @can('asset-manager.manage')
                    <x-ui-button variant="secondary-ghost" size="sm" rounded="lg" wire:click="$toggle('showBulkCreate')">
                        @svg('heroicon-o-square-3-stack-3d', 'w-3.5 h-3.5')
                        Mehrere anlegen
                    </x-ui-button>
                    <x-ui-button variant="primary" size="sm" rounded="lg"
    ...
    <x-slot name="activity">
        <x-ui-page-sidebar title="Aktionen" icon="heroicon-o-bolt" width="w-80" :defaultOpen="false" storeKey="activityOpen" side="right">
            <div class="p-4 space-y-3 bg-[var(--ui-muted-5)]">
                @can('asset-manager.manage')
                    <x-ui-button variant="primary" size="md" rounded="lg" class="w-full"
                                 href="{{ route('asset-manager.assets.create') }}" wire:navigate>
                        @svg('heroicon-o-plus', 'w-4 h-4')
                        Neues Asset anlegen
                    </x-ui-button>
                    <x-ui-button variant="secondary-ghost" size="md" rounded="lg" class="w-full" wire:click="$toggle('showBulkCreate')">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div class="rounded-xl bg-white/80 dark:bg-white/5 backdrop-blur-xl border border-violet-500/30 shadow-sm p-5">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                            @svg('heroicon-o-square-3-stack-3d', 'w-4 h-4 text-[color:var(--ui-primary)]')
                            Mehrere identische Assets anlegen
                        </h2>
                        <button wire:click="$set('showBulkCreate', false)" class="text-[color:var(--ui-secondary)] hover:text-gray-600">@svg('heroicon-o-x-mark', 'w-4 h-4')</button>
                    </div>
                    <form wire:submit="createBulk" class="space-y-3">
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                            ...
                            <div>
                                <label class="block text-[10px] uppercase tracking-wider text-[color:var(--ui-secondary)] mb-1">Anzahl *</label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                            ...
                            <div>
                                <label class="block text-[10px] uppercase tracking-wider text-[color:var(--ui-secondary)] mb-1">Kaufpreis (€/Stück)</label>
                        ...
                        <div class="flex items-end gap-3">
                            <div class="flex-1 max-w-xs">
                                <label class="block text-[10px] uppercase tracking-wider text-[color:var(--ui-secondary)] mb-1">Direkt zuweisen an</label>
                            ...
                            <x-ui-button type="submit" variant="primary" size="md" rounded="lg">
                                @svg('heroicon-o-check', 'w-4 h-4')
                                <span>{{ $bcQuantity > 1 ? $bcQuantity . '× anlegen' : 'Anlegen' }}</span>
                            </x-ui-button>
                        </div>
                        <p class="text-[10px] text-[color:var(--ui-secondary)]">Seriennummern bleiben leer und können später pro Gerät ergänzt werden.</p>
                    </form>
                </div>
            @endif
            @endcan
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="rounded-xl bg-white/60 dark:bg-white/5 border border-black/5 dark:border-white/10 shadow-sm p-4">
                    ...
                    <div class="text-xs text-[color:var(--ui-secondary)]">Assets gesamt</div>
                </div>
                <div class="rounded-xl bg-white/60 dark:bg-white/5 border border-black/5 dark:border-white/10 shadow-sm p-4">
                    <div class="text-2xl font-semibold text-emerald-700 dark:text-emerald-400">{{ $stats['assigned'] }}</div>
                    <div class="text-xs text-[color:var(--ui-secondary)]">Zugewiesen</div>
                </div>
                <div class="rounded-xl bg-white/60 dark:bg-white/5 border border-black/5 dark:border-white/10 shadow-sm p-4">
                    <div class="text-2xl font-semibold text-sky-700 dark:text-sky-400">{{ $stats['in_stock'] }}</div>
                    <div class="text-xs text-[color:var(--ui-secondary)]">Auf Lager</div>
                </div>
                <div class="rounded-xl bg-white/60 dark:bg-white/5 border border-black/5 dark:border-white/10 shadow-sm p-4">
                    <div class="text-2xl font-semibold text-[color:var(--ui-secondary)]">{{ $stats['retired'] }}</div>
                    <div class="text-xs text-[color:var(--ui-secondary)]">Ausgemustert</div>
                </div>
            </div>
            ...
            <div class="rounded-xl bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-black/5 dark:border-white/10 shadow-sm overflow-hidden">
                @if($items->isEmpty())
                    <div class="flex flex-col items-center justify-center py-16 text-center">
                        @svg('heroicon-o-cube-transparent', 'w-10 h-10 text-[color:var(--ui-muted)] mb-3')
                        <p class="text-sm text-[color:var(--ui-secondary)] mb-3">
                            @if($search || $filterCategory || $filterStatus || $filterSource || $filterAssignee)
                        ...
                        @if(!$search && !$filterCategory && !$filterStatus && !$filterSource && !$filterAssignee)
                            @can('asset-manager.manage')
                                <x-ui-button variant="primary" size="md" rounded="lg"
                                             href="{{ route('asset-manager.assets.create') }}" wire:navigate>
                                    @svg('heroicon-o-plus', 'w-4 h-4')
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-[color:var(--ui-muted)] bg-[color:var(--ui-muted-10)]">
                                ...
                                <th class="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-[color:var(--ui-body-color)]">Kategorie</th>
                                <th class="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-[color:var(--ui-body-color)]">Zugewiesen an</th>
                                <th class="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-[color:var(--ui-body-color)]">Status</th>
                                <th class="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-[color:var(--ui-body-color)]">Herkunft</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-[color:var(--ui-muted)]">
                            @foreach($items as $item)
                                @php $isChecked = in_array((string) $item->id, $selected, true); @endphp
                                <tr wire:key="item-{{ $item->id }}" class="transition-colors {{ $isChecked ? 'bg-[color:var(--ui-primary-10)] shadow-[inset_3px_0_0_rgb(var(--ui-primary-rgb))]' : 'hover:bg-[color:var(--ui-muted-10)]' }}">
                                    @can('asset-manager.manage')
                                    <td class="px-5 py-3">
                                    ...
                                    <td class="px-5 py-3">
                                        ...
                                        @if($item->manufacturer || $item->model)
                                            <div class="text-xs text-[color:var(--ui-secondary)]">{{ trim($item->manufacturer . ' ' . $item->model) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-5 py-3 text-gray-700 dark:text-gray-300">
                                        <span class="inline-flex items-center gap-1.5">
                                            @if($item->category?->icon) @svg($item->category->icon, 'w-3.5 h-3.5 text-[color:var(--ui-secondary)]') @endif
                                            {{ $item->category?->name ?? '—' }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-3 text-gray-700 dark:text-gray-300">
