    ...
    @foreach($areas as $key => $a)
        <button type="button" wire:click="setActive('{{ $key }}')"
                class="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-left transition-colors
                       {{ $active === $key ? 'bg-[var(--am-primary)] text-[var(--am-on-primary)] shadow-sm' : 'text-[var(--am-text-secondary)] hover:bg-[var(--am-bg)]' }}">
            @svg($a['icon'], 'w-4 h-4 flex-shrink-0')
            <span class="flex-1 truncate">{{ $a['label'] }}</span>
            <span class="text-[10px] rounded-full px-2 py-0.5 {{ $active === $key ? 'bg-white/20 text-[var(--am-on-primary)]' : 'bg-[var(--am-bg)] text-[var(--am-text-muted)]' }}">{{ $counts[$key] ?? 0 }}</span>
        </button>
    @endforeach
</nav>
