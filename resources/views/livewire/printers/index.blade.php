        ...
        <x-ui-page-sidebar title="Filter" icon="heroicon-o-funnel" width="w-72" :defaultOpen="true">
            <div class="p-4 space-y-4 bg-[var(--ui-muted-5)]">
                ...
                @if($niederlassungen->isNotEmpty())
                    <section class="rounded-lg bg-white border border-[var(--ui-border)]/40 shadow-sm overflow-hidden">
                        <h3 class="text-[10px] font-semibold uppercase tracking-wider text-[var(--ui-muted)] px-3 pt-3 pb-1.5">Niederlassung</h3>
                        <div class="px-3 pb-3">
            ...
            <div class="p-4 space-y-3 bg-[var(--ui-muted-5)]">
                @if($selectedItem)
                    ...
                    <section class="rounded-lg bg-white border border-[var(--ui-border)]/40 shadow-sm p-3">
                        ...
                        <div class="text-[11px] text-[var(--ui-muted)]">{{ $selectedItem->model ?? '—' }}</div>
                    </section>
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p class="text-[11px] text-[var(--ui-muted)]">Einen Drucker in der Liste anklicken.</p>
                    </div>
                @endif
            </div>
        </x-ui-page-sidebar>
    ...
    <div class="flex-1 flex flex-col min-h-0 min-w-0">
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div class="rounded-xl bg-white/60 border border-black/5 shadow-sm overflow-hidden">
                @if($items->isEmpty())
                    <div class="p-8 text-center text-sm text-gray-400">Keine Drucker gefunden.</div>
                @else
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-black/5 text-xs uppercase tracking-wider text-gray-400">
                                <th class="text-left px-4 py-3">Drucker</th>
                                <th class="text-left px-4 py-3">Niederlassung</th>
                        ...
                        <tbody class="divide-y divide-black/[0.03]">
                            @foreach($items as $item)
                                <tr wire:key="pr-{{ $item->id }}" wire:click="selectItem({{ $item->id }})"
                                    ...
                                    <td class="px-4 py-2.5 text-xs text-gray-500">{{ $item->raw_data['niederlassung'] ?? '—' }}</td>
                                    <td class="px-4 py-2.5 text-xs text-gray-500 font-mono">{{ $item->serial_number ?? '—' }}</td>
                                    <td class="px-4 py-2.5 text-xs text-gray-500">{{ $kstByItem[$item->id] ?? '—' }}</td>
