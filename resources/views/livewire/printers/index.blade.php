                        ...
                        <tbody class="divide-y divide-[color:var(--am-border)]">
                            @foreach($items as $item)
                                {{-- Zeilenklick öffnete früher ein rechtes Vorschau-Panel; seit S8b führt der
                                     Name direkt auf die vereinte Detailseite (ADR 0012) — dieselbe Information,
                                     nur vollständig statt als Ausschnitt. --}}
                                <tr wire:key="pr-{{ $item->id }}" class="transition-colors hover:bg-[var(--am-bg)]">
                                    <td class="px-4 py-2.5">
                                        ...
                                        @if($item->model)
                                            <div class="text-xs text-[var(--am-text-muted)]">{{ $item->model }}</div>
                                        @endif
                                    </td>
