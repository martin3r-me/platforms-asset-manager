...
Alias: asset-manager.dashboard
    ↓
Verwendung: <livewire:asset-manager.dashboard />
```
...
### Schritt 1: Kopieren
```bash
cp -r asset-manager dein-modul-name
```

### Schritt 2: Suchen & Ersetzen

**In ALLEN Dateien ersetzen:**
- `AssetManager` → `DeinModulName` (PascalCase, Namespace)
- `asset-manager` → `dein-modul-name` (kebab-case, Routes, Config)
- `module_template` → `dein_modul_name` (snake_case, Config-Keys)

**Wichtige Dateien:**
- `composer.json` - Name, Namespace, Provider
- `config/asset-manager.php` → umbenennen & anpassen
- `src/AssetManagerServiceProvider.php` → umbenennen & anpassen
