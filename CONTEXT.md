...
- **Mobilfunk** — Mobilfunkverträge werden **nicht** als eigenes Modell geführt (ADR 0014). Die **Kosten** sind eine [[Kostenposition]] der Kostenart `mobilfunk` (`aggregation_source='cost_line'`, `per_employee`, Default-Kreditor Vodafone) — je Rufnummer/Mitarbeiter eine Zeile, Laufzeit über `valid_from`/`valid_to`. **Rufnummer** (aus Entra `mobilePhone`/`businessPhones`), **SIM-Nummer**, **Vertragsnummer** und **Datenvolumen** sind **Felder am `AssetEmployee`** (1:1 — eine Nummer je Person, keine Pool-SIMs). `phone_overridden` schützt eine manuell korrigierte Nummer vor dem nächsten Sync. _Avoid_: „Mobilfunkvertrag" als eigene Entität, `aggregation_source='contract'` (würde dieselben Kosten doppelt zählen — verletzt ADR 0001).
- **Entra-Anreicherung** — Der Tenant-User-Sync (`ImportTenantUsersJob` über Graph `/users`) füllt am Mitarbeiter **Abteilung** (`department`), **Position** (`job_title`) und die Rufnummern. **Entra ist führend**: nicht-leere Graph-Werte überschreiben bei jedem Sync, leere lassen den Bestand unangetastet (kein Datenverlust durch lückenhaftes Entra). Benötigt nur `User.Read.All` (bereits erteilt); Rufnummern zusätzlich über `phone_overridden` manuell übersteuerbar.
